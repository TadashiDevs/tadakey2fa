...
class SidebarProvider {
    constructor(_extensionUri, _secrets) {
    ...
    async _initializeState() {
        ...
        if (vaultDataStr) {
            try {
                this._vaultData = JSON.parse(vaultDataStr);
            }
            catch {
                this._vaultData = null;
            }
        ...
        if (!this._systemKey || !this._vaultData) {
            this._systemKey = security_1.Security.generateSystemKey();
    ...
    async _sendSetupState() {
        const secret = security_1.Security.generateTotpSecret();
        const qrCode = await security_1.Security.generateQrCode(secret);
        this._pendingTotpSecret = secret;
    ...
    async _handleMessage(message) {
        switch (message.command) {
            case 'ready':
                ...
                break;
            case 'setup':
                await this._handleSetup(message);
                break;
            case 'unlock':
                await this._handleUnlock(message);
                break;
            case 'resetupTotp':
                await this._handleResetupTotp(message);
                break;
            case 'getSecurityQuestion':
                this._handleGetSecurityQuestion();
                break;
            case 'add':
                await this._handleAddKey(message);
                break;
            case 'view':
                this._handleViewKey(message.id);
                break;
            case 'copy':
                await this._handleCopyKey(message.id);
                break;
            case 'delete':
                await this._handleDeleteKey(message.id);
                break;
            case 'pin':
                await this._handlePinKey(message.id);
                break;
            case 'lock':
                this._handleLock();
                break;
    ...
    async _handleSetup(message) {
        const pendingSecret = this._pendingTotpSecret;
        if (!pendingSecret) {
            this._sendMessage({ command: 'error', message: 'Configuration error. Please reload.' });
            return;
        }
        if (!security_1.Security.validateTotpToken(message.totpCode, pendingSecret)) {
            this._sendMessage({ command: 'error', message: 'Invalid 2FA code.' });
            return;
        }
        if (!message.securityQuestion.trim() || !message.securityAnswer.trim()) {
            this._sendMessage({ command: 'error', message: 'Security Q&A required.' });
            return;
        }
        const salt = security_1.Security.generateSalt();
        this._vaultData = {
            totpSecret: security_1.Security.encrypt(pendingSecret, this._systemKey),
            securityQuestion: message.securityQuestion.trim(),
            securityAnswerHash: security_1.Security.hashSecurityAnswer(message.securityAnswer, salt),
            securitySalt: salt,
            keys: []
        };
        await this._saveVaultData();
        delete this._pendingTotpSecret;
    ...
    async _handleUnlock(message) {
        if (!this._vaultData || !this._systemKey) {
            this._sendMessage({ command: 'error', message: 'No vault.' });
            return;
        }
        if (message.method === 'totp') {
            const totpSecret = security_1.Security.decrypt(this._vaultData.totpSecret, this._systemKey);
            if (!security_1.Security.validateTotpToken(message.value, totpSecret)) {
                this._sendMessage({ command: 'error', message: 'Invalid 2FA code.' });
                return;
            }
            this._isUnlocked = true;
            ...
            if (!security_1.Security.verifySecurityAnswer(message.value, this._vaultData.securityAnswerHash, this._vaultData.securitySalt)) {
                this._sendMessage({ command: 'error', message: 'Incorrect answer.' });
                return;
            }
    ...
    async _handleResetupTotp(message) {
        const pendingSecret = this._pendingTotpSecret;
        if (!pendingSecret || !this._pendingRecovery || !this._vaultData) {
            this._sendMessage({ command: 'error', message: 'Recovery error.' });
            return;
        }
        if (!security_1.Security.validateTotpToken(message.totpCode, pendingSecret)) {
            this._sendMessage({ command: 'error', message: 'Invalid code.' });
            return;
        }
        this._vaultData.totpSecret = security_1.Security.encrypt(pendingSecret, this._systemKey);
        await this._saveVaultData();
        delete this._pendingTotpSecret;
        this._pendingRecovery = false;
        this._isUnlocked = true;
    ...
    _handleGetSecurityQuestion() {
        if (this._vaultData) {
            this._sendMessage({ command: 'securityQuestion', question: this._vaultData.securityQuestion });
            this._sendState('recovery');
        }
    }
    async _handleAddKey(message) {
        if (!this._isUnlocked || !this._vaultData || !this._systemKey) {
            this._sendMessage({ command: 'error', message: 'Locked.' });
            return;
        }
        if (!message.name.trim()) {
            this._sendMessage({ command: 'error', message: 'Name required.' });
            return;
        }
        if (!message.value.trim()) {
            this._sendMessage({ command: 'error', message: 'Value required.' });
            return;
        }
        if (message.type === 'login' && !message.username?.trim()) {
            this._sendMessage({ command: 'error', message: 'Username required.' });
            return;
        }
        const newKey = {
            id: security_1.Security.generateId(),
            type: message.type,
            name: message.name.trim(),
            encryptedValue: security_1.Security.encrypt(message.value, this._systemKey),
    ...
    _handleViewKey(id) {
        ...
        if (key)
            this._sendMessage({ command: 'revealed', id, value: security_1.Security.decrypt(key.encryptedValue, this._systemKey) });
    }
    async _handleCopyKey(id) {
        if (!this._isUnlocked || !this._vaultData || !this._systemKey)
            return;
        const key = this._vaultData.keys.find(k => k.id === id);
        if (key) {
            await vscode.env.clipboard.writeText(security_1.Security.decrypt(key.encryptedValue, this._systemKey));
            this._sendMessage({ command: 'copied', id });
            this._sendMessage({ command: 'success', message: 'Copied!' });
        }
    }
    async _handleDeleteKey(id) {
        if (!this._isUnlocked || !this._vaultData)
            return;
    ...
    async _saveVaultData() {
        if (this._vaultData)
            await this._secrets.store(VAULT_DATA_STORAGE, JSON.stringify(this._vaultData));
    }
    _sendState(state) { this._sendMessage({ command: 'state', state }); }
    _sendKeys() {
        if (this._vaultData) {
            const keys = this._vaultData.keys.map(k => ({ id: k.id, type: k.type || 'apikey', name: k.name, username: k.username, pinned: k.pinned || false }));
            this._sendMessage({ command: 'keys', keys });
        }
    }
    _sendMessage(message) { this._view?.webview.postMessage(message); }
...
SidebarProvider.viewType = 'tadakey2fa';
